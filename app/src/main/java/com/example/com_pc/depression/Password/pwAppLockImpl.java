...
public class pwAppLockImpl extends pwAppLock implements pwPageListener {
	...
	public pwAppLockImpl(Application app) {
		super();
	...
	public void enable() {
		pwBaseActivity.setListener(this);
	}

	public void disable() {
		pwBaseActivity.setListener(null);
	}

	public boolean checkPasscode(String passcode) {
		passcode = PASSWORD_SALT + passcode + PASSWORD_SALT;
		passcode = pwEncryptor.getSHA1(passcode);
	...
	private boolean shouldLockSceen(Activity activity) {
		...
		if (activity instanceof AppLockActivity) {
			...
			if (ala.getType() == pwAppLock.UNLOCK_PASSWORD) {
	...
	public void onActivityResumed(Activity activity) {
		...
		if (shouldLockSceen(activity)) {
			...
			intent.putExtra(pwAppLock.TYPE, pwAppLock.UNLOCK_PASSWORD);
